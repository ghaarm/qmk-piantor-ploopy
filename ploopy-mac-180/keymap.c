...
void pointing_device_gestures_trigger(uint8_t direction) {
    ...
    switch( direction ) {
        case 0: /* East, Next Virtual Desktop. */
            ...
            /* Old linux virtual desktop entry On Linux, send CTRL + ALT + RIGHT. */
            /*else if( detected_host_os() == OS_LINUX ) {
                tap_code16( LCA( KC_RIGHT ) );
            }*/
            /* On SwayWM on Fedora, send CTRL + LGUI + S. */
            else if( detected_host_os() == OS_LINUX ) {
                tap_code16( LCG( KC_S ) );
            }
        ...
        case 4: /* West, Previous Virtual Desktop. */
            ...
            /* Old Linux, send CTRL + ALT + LEFT. */
            /*else if( detected_host_os() == OS_LINUX ) {
                tap_code16( LCA( KC_LEFT ) );
            }*/
            /* On SwayWM on Fedora Linux, send CTRL + LGUI + H. */
            else if( detected_host_os() == OS_LINUX ) {
                tap_code16( LCG( KC_H ) );
            }
...
/*   old right-handed layout for not-rotated keymap
...
*/
    // Top Knob rotated Base layer with all of the mouse-related stuff for everyday use.
    [LAYER_NAV_RIGHT_HANDED] = LAYOUT(  MS_BTN3, TG(LAYER_CONTROL), PKC_GESTURE, MS_BTN1, 
                                        MS_BTN2, MS_BTN4, 
                                        PKC_DRAG_SCROLL, MS_BTN5 ),
/*  Old left hand layout for non-rotated keymap
...
*/
    // Top Knob rotated Mirror-image of right-handed layout for lefties.
    [LAYER_NAV_LEFT_HANDED] = LAYOUT(  MS_BTN1, PKC_GESTURE, TG(LAYER_CONTROL), MS_BTN3, 
                                        MS_BTN4, MS_BTN2, 
                                        MS_BTN5, PKC_DRAG_SCROLL ),
/*  Old control layout for non-rotated keymap
...
*/
    // Top Knob rotated Layer for all of the customization options.
    [LAYER_CONTROL] = LAYOUT(           PKC_TGL_DRAG_SCRL, TG(LAYER_CONTROL), TG(LAYER_CONTROL), PKC_TGL_MIRROR,
                                        PKC_TGL_HORIZ_SCRL, PKC_BLINKY_DPI_CONFIG, 
                                        PKC_TGL_VERT_SCRL,  PKC_ADJUST_LED_BRIGHTNESS ),
